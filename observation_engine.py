...
class ObservationWindowManager:
    ...
    async def evaluate(self, symbol: str) -> Optional[CandidateObservation]:
        """Runs one fast-scalp check for `symbol`. Returns the candidate
        once it's ACCEPTED (removed from tracking immediately after).
        Returns None on every other tick — including a tick where trend
        or pressure/volume don't currently qualify, which simply leaves
        the candidate OBSERVING to be re-checked fresh next tick, with no
        memory of the failed check carried forward."""
        cfg = self.config
        async with self._lock:
